...
const AboutPage: React.FC = () => {
  ...
  // Control slideshow based on visibility
  useEffect(() => {
    if (isInView) {
      // Start slideshow when page is in view
      setIsSlideshowActive(true);
    } else {
      // Stop slideshow when page is not in view
  ...
  useEffect(() => {
    ...
    if (isSlideshowActive) {
      // Only start the interval if the slideshow is active
      interval = setInterval(() => {
        ...
        setCurrentSlide((prev) => {
          ...
          if (next >= displayedSlides.length - sliderContent.length) {
            // Add more slides to the end to continue the infinite scroll
    ...
    // Clean up interval when component unmounts or slideshow becomes inactive
    return () => {
      if (interval) clearInterval(interval);
    };
  }, [isSlideshowActive, displayedSlides.length]);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#f8f9fa]" ref={sectionRef}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            ...
            >
              ...
              <div className="space-y-6 text-dark/70">
                <p>
                  Welcome to UI Mitra, where innovation meets excellence. We specialize in delivering cutting-edge digital solutions 
                ...
                <p>
                  At UI Mitra, we believe in the power of technology to transform businesses and create meaningful connections. 
